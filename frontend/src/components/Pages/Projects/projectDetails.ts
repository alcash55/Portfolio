...
export const projectDetails: Partial<Record<string, ProjectDetail>> = {
  'Portfolio Website': {
    ...
    facts: [
      ...
      'Projects API: 1h cache, single-flight refresh, 0.54s cold and 0.0003s warm',
  ...
  'The Cliper-er': {
    ...
    sections: [
      ...
      },
      {
        heading: 'What was hard',
        body: [
          'Deciding what is in the frame, because the obvious signals do not work. Facecam detection is the clearest case: IRL streams kept triggering the split-screen layout meant for a webcam, since on an IRL stream the whole frame *is* the camera. The two signals I expected to separate them, face scale and boundary continuity, did not, measured across all 15 real clips I had. What did was crowd density: IRL footage runs 3.7 to 10.2 face detections per frame against 0.6 to 2.6 for a genuine facecam. A threshold at 3.0 passes all five true facecams and rejects all five IRL clips.',
          'The second hard part was not a code problem at all. Thirteen uploads in a row got zero views, and reading YouTube\'s own policy text rather than the SEO blogs pointed at "inauthentic content", meaning templated output produced at scale, more than at reused content. Cropping and captioning are production polish, not a narrative, which is the bar the policy actually describes. So the pipeline now writes a per-clip story line and an on-screen hook card, and hashtags went from padding to fifteen with generic filler down to three to five real ones with `#shorts` dropped, since YouTube has classified Shorts by aspect ratio and duration since 2023 and that tag is now a wasted slot.',
        ],
      },
      {
        heading: "What I'd do differently",
        body: [
          ...
          'I would also put the repo under git before the second overhaul rather than after it, and I would treat "the platform is suppressing me" as the last hypothesis instead of the first. The check that would have settled it, the per-video restriction notices in YouTube Studio, takes about a minute and rules out the interesting explanation for free.',
        ],
      },
    ],
    tech: [
